object Day8 extends App {
  ...
  println("answer1=" + digits.map{x => x.split(' ').map{x => x.length}}.flatten.filter{x => x==2 || x==4 || x==3 || x==7}.size)
  ...
  def deduce(rem: List[Set[Char]], acc: Map[Int, Set[Char]]): Map[Int, Set[Char]] = {
    ...
    if (rem.isEmpty)
       ...
       else if (acc.isDefinedAt(3) && acc.isDefinedAt(6) && acc.isDefinedAt(4)) {
